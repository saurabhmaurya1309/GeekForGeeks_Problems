...
class Solution{
  public:
        int i=0;
    Node*solve(int pre[],int preMirror[],int n){
        if(i==n){
            return NULL;
        }
        int j=0;
        for(j;j<n;j++){
            ...
            if(j==n-1||preMirror[j+1]==-1){
                return root;
            }
            root->left=solve(pre,preMirror,n);
            root->right=solve(pre,preMirror,n);
            return root;
